...
export function useEditableTodo(initialText: string, initialDueDate?: number) {
  ...
  const [editingDueDate, setEditingDueDate] = useState<Date | null>(initialDueDate ? new Date(initialDueDate) : null);
  ...
  const cancelEdit = () =>{
    ...
    const commitTodoText = (onSave: (value : string, dueDate?: number)=> void) =>{
